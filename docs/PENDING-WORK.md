...
| # | Item | Detail |
|---|------|--------|
| ⚪ 4.1 | Delete legacy push path | **DONE (2026-06-09):** deleted orphaned `modules/ops_client/push.py` (`push_apparel_product`, zero prod callers) + `tests/test_ops_client_push.py`. Per-fn dead-code pass completed on `mutations.py`: removed the dead wrappers (`set_product_category`, `set_products_image_gallery`, `set_assign_options`, `set_additional_option`, `set_additional_option_attributes`, `set_products_attribute_price`, `set_product_design`) whose only callers were the deleted `tests/test_ops_mutations.py`; deleted that test file. Kept the `_SET_*`/`_UPDATE_*` query consts (gateway dispatches them directly), `get_product_by_sku` (live in `gateway.py` dedup/verify + `verify.py`), and the 4 reference wrappers (`set_product`, `set_product_size`, `set_product_price`, `update_product_stock`) — their silent-failure (`result:false`) regression coverage lives in `test_ops_push_normalization.py`. Unit suite green (112 passed; 719 collect clean; DB-backed tests error only on missing local Postgres per §5.3). |
