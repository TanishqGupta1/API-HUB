		...
		for (let i = 0; i < items.length; i++) {
			...
			if (resource === 'customerAddress' && operation === 'getAll') {
				...
				if (resource === 'batch' && operation === 'getAll') {
					const variables: IDataObject = {};
					const batchId = this.getNodeParameter('batch_batchId', i) as number;
					const search = this.getNodeParameter('batch_search', i) as string;
					const limit = this.getNodeParameter('batch_limit', i) as number;
					const offset = this.getNodeParameter('batch_offset', i) as number;
					if (batchId) variables.batch_id = batchId;
					...
					if (responseData && responseData.data && responseData.data.getBatch) {
						...
						for (const b of batches) { returnData.push({ ...b, _totalBatchDetails: responseData.data.getBatch.totalBatchDetails }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`);
					}
				}
				...
				if (resource === 'quote' && operation === 'getAll') {
					const variables: IDataObject = {};
					const quoteId = this.getNodeParameter('quote_quoteId', i) as number;
					const userId = this.getNodeParameter('quote_userId', i) as number;
					const limit = this.getNodeParameter('quote_limit', i) as number;
					const offset = this.getNodeParameter('quote_offset', i) as number;
					if (quoteId) variables.quote_id = quoteId;
					...
					if (responseData && responseData.data && responseData.data.get_quote) {
						...
						for (const q of quotes) { returnData.push({ ...q, _totalQuote: responseData.data.get_quote.totalQuote }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`);
					}
				}
				...
				if (resource === 'quoteProduct' && operation === 'getAll') {
					const variables: IDataObject = {};
					const quoteId = this.getNodeParameter('quoteProduct_quoteId', i) as number;
					const quoteProductsId = this.getNodeParameter('quoteProduct_quoteProductsId', i) as number;
					const limit = this.getNodeParameter('quoteProduct_limit', i) as number;
					const offset = this.getNodeParameter('quoteProduct_offset', i) as number;
					if (quoteId) variables.quote_id = quoteId;
					if (quoteProductsId) variables.quote_products_id = quoteProductsId;
					if (limit) variables.limit = limit;
					if (offset) variables.offset = offset;
					...
					if (responseData && responseData.data && responseData.data.quoteproduct) {
						...
						for (const p of products) { returnData.push({ ...p, _totalQuoteProduct: responseData.data.quoteproduct.totalQuoteProduct }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`);
					}
				}
				...
				if (resource === 'department' && operation === 'getAll') {
					...
					const corporateId = this.getNodeParameter('department_corporateId', i) as number;
					const limit = this.getNodeParameter('department_limit', i) as number;
					const offset = this.getNodeParameter('department_offset', i) as number;
					if (departmentId) variables.department_id = departmentId;
					if (corporateId) variables.corporate_id = corporateId;
					if (limit) variables.limit = limit;
					if (offset) variables.offset = offset;
					...
					if (responseData && responseData.data && responseData.data.get_departments) {
						...
						for (const d of departments) { returnData.push({ ...d, _totalDepartments: responseData.data.get_departments.totalDepartments }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`);
					}
				}
				...
				if (resource === 'mutation') {
					...
					if (operation === 'setOrderProduct') {
						...
						const responseData = await this.helpers.request({ method: 'POST', url: `${baseUrl}/api/`, headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: { query: mutation, variables }, json: true });
						if (responseData && responseData.data && responseData.data.setOrderProduct) returnData.push(responseData.data.setOrderProduct);
						else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`);
					}
					if (operation === 'setBatch') {
						...
						const responseData = await this.helpers.request({ method: 'POST', url: `${baseUrl}/api/`, headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: { query: mutation, variables }, json: true });
						if (responseData && responseData.data && responseData.data.setBatch) returnData.push(responseData.data.setBatch);
						else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`);
					}
					...
					if (operation === 'setQuote') {
						const userid = this.getNodeParameter('setQuote_userid', i) as number;
					...
					if (operation === 'setProductDesign') {
						...
						const ziflow_preflight_link = this.getNodeParameter('setProductDesign_ziflow_preflight_link', i) as string;
						const variables: IDataObject = {};
						if (order_product_id) variables.order_product_id = order_product_id;
						if (ziflow_link) variables.ziflow_link = ziflow_link;
						if (ziflow_preflight_link) variables.ziflow_preflight_link = ziflow_preflight_link;
						...
						const responseData = await this.helpers.request({ method: 'POST', url: `${baseUrl}/api/`, headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: { query: mutation, variables }, json: true });
						if (responseData && responseData.data && responseData.data.setProductDesign) returnData.push(responseData.data.setProductDesign);
						else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`);
					}
