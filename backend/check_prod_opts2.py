...
query GetOpts($pid: Int) {
  productAdditionalOptions(products_id: $pid, limit: 50, offset: 0) {
...
async def main():
    ...
    async with OpsGraphQLClient(auth) as ops:
        ...
        if not res.ok:
            ...
            return
        opts = (res.data or {}).get('productAdditionalOptions', {}).get('productAdditionalOptions', [])
        print(f"Product 612 has {len(opts)} additional options:")
        for o in opts:
            print(f"  prod_add_opt_id={o['prod_add_opt_id']} master_option_id={o.get('master_option_id')} title={o['title']!r}")
