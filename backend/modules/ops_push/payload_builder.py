...
def _canonical_number(n: Union[int, float]) -> str:
    ...
    shortest-round-trip rule that ECMA-262 + RFC 8785 mandate. The
    edge cases (negative-zero, exponent format) are normalized below.
    """
...
def _canonical_value(v: Any) -> str:
    ...
    if isinstance(v, dict):
        # Lex-sort by key (UTF-16 code units; for typical ASCII/UTF-8 keys
        # Python's str ordering matches UTF-16 code unit ordering).
