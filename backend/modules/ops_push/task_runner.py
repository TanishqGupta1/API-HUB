...
# Track active slot count explicitly — sem._value is a CPython private
# attribute and not part of the public API.
_active_count: int = 0
...
async def run_push_task(push_log_id: uuid_mod.UUID) -> None:
    ...
    from .gateway import execute_push  # local import to avoid circular deps  # TODO: resolve circular import

    sem = _get_semaphore()
    log.info("push_task queued  push_log=%s  (active=%s/%s)",
             push_log_id, _active_count, _MAX_CONCURRENT)

    async with sem:
        _active_count += 1
        try:
            log.info("push_task started push_log=%s  (active=%s/%s)",
                     push_log_id, _active_count, _MAX_CONCURRENT)
            try:
