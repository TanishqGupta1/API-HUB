...
class FakeOpsClient:
    ...
    can swap between real and fake by injection. Allocates monotonic
    synthetic IDs starting at 1000 (visually distinct from real OPS IDs
    when reading test output) and records every call on `self.calls` so
    tests can assert the exact mutation sequence.
    """
