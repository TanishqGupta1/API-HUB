...
async def _create_supplier(client: AsyncClient) -> str:
    ...
    _assert_not_redirect(resp)
    assert resp.status_code == 201, resp.text
    return resp.json()["id"]


def _assert_not_redirect(resp) -> None:
    # A 307 from a trailing-slash redirect would have an empty body, making
    # the auth-leak assertions trivially pass. Fail loudly instead so the
    # test never silently skips its real checks.
    assert resp.status_code != 307, (
        f"unexpected 307 redirect to {resp.headers.get('location')!r} — "
        "auth-leak checks would silently skip; fix the test URL"
    )
